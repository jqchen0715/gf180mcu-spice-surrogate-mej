#!/usr/bin/env python3
"""Generate independent SPICE datasets from released SKY130 HD cells."""
...
from spice_v2.sky130_library_cells import selected_cells, simulate_library_point  # noqa: E402


DEFAULT_OUTDIR = PROJECT_ROOT / "results" / "sky130_cross_pdk_replication"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset-name", choices=["primary", "validation"], default="primary")
...
def main() -> int:
    ...
    output_root = args.outdir / ("smoke_all_cells" if args.smoke_test else args.dataset_name)
    netlist_dir = output_root / "netlists"
    log_dir = output_root / "logs"
    tasks = []
    sample_id = 13_000_000 if args.dataset_name == "primary" else 14_000_000
    for cell_index, cell in enumerate(cells):
        ...
        for point_index, (vdd, temp, slew, cload) in enumerate(points):
            if args.smoke_test:
                vdd, temp, slew, cload = 1.8, 25.0, 0.10, 0.02
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
        ...
        for future in as_completed(futures):
            ...
            rows.append(row)
            print(
                f"{row['cell_variant']} {row['corner']} {row['status']} "
                f"delay={row['delay_avg_ns']}",
                flush=True,
            )

    frame = pd.DataFrame(rows).sort_values("sample_id").reset_index(drop=True)
    output_root.mkdir(parents=True, exist_ok=True)
    output = output_root / f"sky130_library_{args.dataset_name}.csv"
